...
-- ============================================================
-- FinApp — Complete Database Setup for Local Development
-- ============================================================
-- Usage:
--   1. Open phpMyAdmin or run:
--      mysql -u root -p < finapp_setup.sql
--   2. Creates database `de2shrnx_finapp`, all tables, and your existing data.
--   3. Then applies multi-user migration (users table + user_id columns).
--   4. Existing data is assigned to the first registered user.
-- ============================================================

-- Database: `de2shrnx_finapp`
...
DROP TABLE IF EXISTS `credit_card_emi_plans`;
...
DROP TABLE IF EXISTS `credit_card_emi_schedule`;
...
DROP TABLE IF EXISTS `credit_card_rewards`;
...
DROP TABLE IF EXISTS `lending_repayments`;
...
DROP TABLE IF EXISTS `loan_emi_schedule`;
...
DROP TABLE IF EXISTS `purchase_sources`;
...
-- Run this AFTER the base dump above.
-- Existing data is assigned to user_id = 1 (the first user).
-- ============================================================
...
-- Placeholder owner — register via /index.php?module=register to get a real bcrypt hash
...
-- 2. Add user_id columns (nullable first so existing rows do not fail)
ALTER TABLE `accounts`         ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `categories`       ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `subcategories`    ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `contacts`         ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `payment_methods`  ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `purchase_sources` ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `reminders`        ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `investments`      ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `properties`       ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `tenants`          ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `loans`            ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `lending_records`  ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
ALTER TABLE `transactions`     ADD COLUMN `user_id` INT UNSIGNED NULL AFTER `id`;
...
-- 4. Make user_id NOT NULL and add FK constraints
...
-- Sessions table for DB-based session storage
